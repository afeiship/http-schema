# http-schema: 在 RequestConfig 中注入接口 schemaKey
...
**目标**：在 `RequestConfig` 顶层注入一个 `schemaKey` 字段，值为该接口的 schema key（即 `item.name`），让拦截器能直接定位当前请求对应的接口。

## 设计

### RequestConfig.schemaKey

在构建给 `httpClient.request(config)` 的 `RequestConfig` 时，自动注入 `schemaKey` 字段，值为解析后的接口名 `item.name`。
...
const config: RequestConfig = {
  ...
  schemaKey: item.name,  // 新增：注入接口 schema key
  ...item.config,
  ...callOptions,
};
```

`schemaKey` 放在 `...item.config` 与 `...callOptions` **之前**。若用户在 DSL 的 `item.config` 或调用时的 `callOptions` 中显式传了 `schemaKey`，则显式值优先，覆盖自动注入值。这是合理行为——用户显式指定优先。
...
const interceptor = () => ({
  id: 'my-interceptor',
  response: (res) => {
    const schemaKey = res.config.schemaKey;
    if (schemaKey === 'categories_root') {
      // 仅针对该接口做特殊处理
    }
    return res;
  },
...
1. 生成的接口函数发起请求时，`config.schemaKey` 等于接口的 schema key。
2. 当 `callOptions` 或 `item.config` 显式传入 `schemaKey` 时，显式值优先。

## 未纳入范围

- 不将 `schemaKey` 作为请求头或参数发送给后端（用户已确认仅 interceptor 内部使用）。
